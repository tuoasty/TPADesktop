...
export default function ViewAllMaintenanceReport() {
    ...
    const acceptSubmission = async (maintenanceId: number) => {
        try {
            await invoke("accept_submission", {selectedMaintenanceId: maintenanceId});
            toast.success("Successfully accepted submission")
            fetchMaintenanceReports();
        } catch (e) {
            toast.error(`${e}`)
        }
    ...
    const rejectSubmission = async (maintenanceId: number) => {
        try {
            await invoke("reject_submission", {selectedMaintenanceId: maintenanceId});
            toast.success("Successfully rejected submission")
            fetchMaintenanceReports();
        } catch (e) {
            toast.error(`${e}`)
        }
                    ...
                    <div key={report.id} className="w-full bg-white h-min-72 rounded-2xl shrink-0 flex justify-between">
                        ...
                        {report.status != "Pending Review" ? (
                            <div className="flex flex-col gap-5 justify-center pr-8">
                                ...
                                }} className="w-48 h-12 bg-red-500"
                                        disabled={report.status == "Completed" || report.status == "Rejected" || report.status == "In Progress"}>
                                    Decline Report
                                </Button>
                            </div>
                            ...
                            <div className="flex flex-col gap-5 justify-center pr-8">
                                <Button onClick={() => {
                                    acceptSubmission(report.id)
                                }} className="w-48 h-12 bg-green-500">
                                    Accept Submission
                                </Button>
                                <Button onClick={() => {
                                    rejectSubmission(report.id)
                                }} className="w-48 h-12 bg-red-500">
                                    Decline Submission
                                </Button>
                            </div>
